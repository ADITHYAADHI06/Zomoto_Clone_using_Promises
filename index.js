let isOrderAccepted = false;  // to accept the order 
...
// when Zomato App - Start 
window.addEventListener('load', function () {
    document.getElementById('accept/reject_Order').addEventListener('click', function () {
        askRestaurantToAcceptOrReject();
    });

    document.getElementById('findValet').addEventListener('click', function () {
        startSearchingFordelivaryPartner();
    })
    ...
    checkIfOrderAcceptedFromRestaurant()
        .then(isOrderAccepted => {
            console.log('updated from restaurant - ', isOrderAccepted ? " order accepted" : " order rejected ");
...
function startPreparingOrder() {
    Promise.allSettled([
        updateOrderStatus(),
        updateMapView(),
        checkIfdelivaryPartnerAssigned(),
    ])
        .then(res => {
            console.log(res);
            setTimeout(() => {
...
function startSearchingFordelivaryPartner() {
    // BED
    ...
    Promise.any(valetsPromises)
        .then(selectedValet => {
            console.log('Selected a delivary Partner => ', selectedValet);
            isValetFound = true;
        })
        .catch(err => {
            console.error(err);
...
function getRandomDriver() {
    ...
    return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            resolve('delivary Partner - ' + timeout);
        }, timeout);
    });
}

function checkIfdelivaryPartnerAssigned() {
    return new Promise((resolve, reject) => {
        valetTimer = setInterval(() => {
            console.log(' searching for delivary Partner');
            if (isValetFound) {
                updateValetDetails();
                resolve('updated delivary Partner details');
...
function checkIfOrderDelievered() {
    return new Promise((resolve, reject) => {
        valetDeliveryTimer = setInterval(() => {
            console.log('is order delivered by delivary Partner');
            if (isOrderDelivered) {
                resolve('order delivered by delivary Partner ');
                updateOrderStatus();
